...
kmer_pair retrieve_from_global_hashMap(uint64_t slot_pos){
  //TODO: OPTIMIZATION
  return rget(global_hashMap + slot_pos).wait();
}

/* -------------------------------- SELF (FAKE) RPC FUNCTIONS -------------------------------- */
uint64_t self_rpc_find_open_slots(){
  //If find an open slot locally, return the index to the rpc callers
  pair<bool, uint64_t> result = myLockerTable.check_slot_available(0);
  if (result.first){
...
uint64_t rpc_find_open_slots(uint64_t slot_pos, int chain_count){
  //If find an open slot locally, return the index to the rpc callers
  pair<bool, uint64_t> result = myLockerTable.check_slot_available(slot_pos);
  if (result.first){
    return result.second + myOffset;
  //Else rpc call the next process
  }else{
    int recipient = (rank_me() + 1) % rank_n();
    uint64_t open_slot_pos;
    if (recipient == rank_me()){
      open_slot_pos = self_rpc_find_open_slots();
    }else{
      open_slot_pos = rpc(recipient, rpc_find_open_slots, 0, chain_count+1).wait();
    }
    return open_slot_pos;
  }
  return -1;
}
...
global_ptr<kmer_pair> create_or_fetch_global_hashMap(){
  global_ptr<kmer_pair> global_hashMap = nullptr;
  if (rank_me() == MASTER){
    global_hashMap = new_array<kmer_pair>(hashMapSize);
  }
  global_hashMap = broadcast(global_hashMap, MASTER).wait();
...
bool insert_kmer(kmer_pair kmer){
  ...
  uint64_t whichSlot = hash % averageOffset;
  //Call RPC
  //TODO: OPTIMIZATION
  uint64_t which_global_hashMap_slot = rpc(whichProc, rpc_find_open_slots, whichSlot, 0).wait();
  if (which_global_hashMap_slot == -1){
    return false;
  }
  //Insert the kmer globally
  //TODO: OPTIMIZATION
  rput(kmer_pair(kmer), global_hashMap + which_global_hashMap_slot).wait();
  return true;
}
...
void houseKeeping(std::string kmer_fname){
  ...
  BUtil::print("Number of kmers: %d \n", n_kmers);
  // Load factor of 0.5
  hashMapSize = n_kmers * (1.0 / 0.5);

  //Initialize variables
  initialize_variables();
  upcxx::barrier();

  //Get global_hashMap pointer
  global_hashMap = create_or_fetch_global_hashMap();
  upcxx::barrier();
...
int main(int argc, char **argv) {
  ...
  if (run_type != "test") {
    BUtil::print("Process %d: finished reading %d kmers.\n", rank_me(), kmer_vocabulary.size());
  }
  ...
  if (run_type != "test") {
    ...
    BUtil::print("Rank %d contigs has a size of %d \n.", rank_me(), contigs.size());
  }

  if (run_type == "test" || run_type == "debug") {
